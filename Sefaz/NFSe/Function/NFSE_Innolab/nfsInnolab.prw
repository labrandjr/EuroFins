...
/*/
//-----------------------------------------------------------------------
user function nfsInnolab( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local cWhere	 := ""
	Local cMunISS	 := ""
	Local cTipoPcc   := "PIS','COF','CSL','CF-','PI-','CS-','IRF','IR-" // "PIS','COF','CSL','CF-','PI-','CS-" // Personalizado
	Local cCodCli 	 := ""
	Local cLojCli 	 := "" 
	Local cDescMunP	 := ""
	local cMunPSIAFI := ""
	...
	Local cRet       := "Servicos de Analises Quimicas "		// Personalizado
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.SF2->F2_DUPL == (cAliasSE1)->E1_NUM .AND.;
					...
					If lNfsePcc
						If Alltrim((cAliasSE1)->E1_TIPO) $ "NF"
							//nRetCsl += (cAliasSE1)->E1_CSLL //Personalizado
							//nRetCof	+= (cAliasSE1)->E1_COFINS //Personalizado
							//nRetPis += (cAliasSE1)->E1_PIS //Personalizado
							nRetCsl += iif((cAliasSE1)->E1_CSLL>=SuperGetMv( "MV_ZZRTCSL"),(cAliasSE1)->E1_CSLL,0)
							nRetCof	+= iif((cAliasSE1)->E1_COFINS>=SuperGetMv( "MV_ZZRTCOF"),(cAliasSE1)->E1_COFINS,0)
							nRetPis += iif((cAliasSE1)->E1_PIS>=SuperGetMv( "MV_ZZRTPIS"),(cAliasSE1)->E1_PIS,0)
						EndIf	
					Endif
					//Else
						If 	(cAliasSE1)->E1_TIPO $ cTipoPcc
							If (cAliasSE1)->E1_TIPO $ "PIS,PI-"
								//nRetPis	+= 	(cAliasSE1)->E1_VALOR //Personalizado
							ElseIf (cAliasSE1)->E1_TIPO $ "COF,CF-"
								//nRetCof	+= 	(cAliasSE1)->E1_VALOR //Personalizado		
							ElseIf (cAliasSE1)->E1_TIPO $ "CSL,CS-"
								//nRetCsl	+= 	(cAliasSE1)->E1_VALOR //Personalizado
							EndIf				 							
						EndIf
					//EndIf	
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lNFeDesc
					...
					*/
					/*
					cAux := MSMM(SC5->C5_CODMEMO)    // CAMPO MEMO --> C5_INFOADI

					If !Empty(cAux)
						cAux += iif(substr(cAux,Len(cAux),1)==".","",".")
		...
		If lFirst
			...
			If Len(aDuplic) > 0
				For nD:=1 to Len(aDuplic)
					cDuplic += "Vencto: "+aDuplic[nD][1]+/*" Valor Liquido: "+Alltrim(Transform(aDuplic[nD][2],"@E 999,999,999.99"))+*/;
...
/*			If SA1->A1_ZZTPCOB == "D" .and. Len(aDuplic) > 0 //removido em 23/05/19 por sergio braz ch. 2019051610036496
			...
			//cMensCli := ""
			cMensCli += "||"+cAux
			cMensCli += "||Local de tributacao : Rio de Janeiro/RJ."
			lFirst := .F.
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				//cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf,aRetSF3 ) //Personalizado
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf,aRetSF3, cMensCli )
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3,cMensCli)	
