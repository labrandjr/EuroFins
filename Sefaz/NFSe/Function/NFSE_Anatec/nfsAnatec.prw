...
#include "topconn.ch"    // Personalizado

///////////
//Buscar pela palavra Personalizado caso seja necessário refazer o fonte
///////////

//-----------------------------------------------------------------------
/*/{Protheus.doc} nfsAnatec
...
/*/
//-----------------------------------------------------------------------
user function nfsAnatec( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local cWhere	 := ""
	Local cMunISS	 := ""
	Local cTipoPcc   := "PIS','COF','CSL','CF-','PI-','CS-','IRF','IR-"//"PIS','COF','CSL','CF-','PI-','CS-','IRF','IR" //Personalizado
	Local cCodCli 	 := ""
	Local cLojCli 	 := "" 
	Local cDescMunP	 := ""
	local cMunPSIAFI := ""
	...
	Local lFirst := .T. 		// Personalizado
	Local aProcessos := {}  	// Personalizado
	Local aInvoices  := {}  	// Personalizado
	Local cProcs     := ""		// Personalizado
	Local cInvs      := ""		// Personalizado
	Local nP         := 0		// Personalizado
	Local nI         := 0		// Personalizado
	Local cDuplic    := ""		// Personalizado
	Local cAux       := ""		// Personalizado
	Local nRetIRRF   := 0		// Personalizado
	Local cLei	   :=SuperGetMV("MV_ZZLEINF", ,"") //Personalizado
	Private aUF     	:= {}         
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.SF2->F2_DUPL == (cAliasSE1)->E1_NUM .AND.;
					...
					If lNfsePcc
						If Alltrim((cAliasSE1)->E1_TIPO) $ "NF"
							...
							nRetCsl += iif((cAliasSE1)->E1_CSLL>=SuperGetMv( "MV_ZZRTCSL"),(cAliasSE1)->E1_CSLL,0)//Personalizado
							nRetCof	+= iif((cAliasSE1)->E1_COFINS>=SuperGetMv( "MV_ZZRTCOF"),(cAliasSE1)->E1_COFINS,0)//Personalizado
							nRetPis += iif((cAliasSE1)->E1_PIS>=SuperGetMv( "MV_ZZRTPIS"),(cAliasSE1)->E1_PIS,0)//Personalizado
						EndIf	
					//Else
					endif
						If 	(cAliasSE1)->E1_TIPO $ cTipoPcc
							...
							ElseIf (cAliasSE1)->E1_TIPO $ "IRF,IR-"
								nRetIRRF += (cAliasSE1)->E1_VALOR
								// Personalizado Fim
							EndIf				 							
						EndIf
					//EndIf	
			...
			*/
			If nRetIRRF > 0
				aadd(aRetido,{"IRRF",0,nRetIRRF,SED->ED_PERCIRF,aRetIRR})
			Endif
			//Personalzado - fim
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				aAdd(aRetIRR,Iif(SF2->(FieldPos("F2_VALIRRF")) <> 0 .and. nRetIRRF > 0, (cAliasSD2)->D2_VALIRRF, 0))
				// Personalizado - fim
				...
				If nScan > 0
					...
					*/
					If lMvNFSEIR .and. nCont == 1
						aRetido[nScan][3] := nRetIRRF
					EndIf
					// Personalizado - fim
				...
				If !lNFeDesc
					...
					EndIf
					*/
					If aScan(aProcessos , Alltrim(SC5->C5_ZZNROCE)) == 0
						aadd(aProcessos , Alltrim(SC5->C5_ZZNROCE))
					Endif
					//					If SM0->M0_CODIGO == '05' //ANNATEC
					If Alltrim(SM0->M0_CODFIL) $ '0500|0501|0502|0503|0504' //ANNATEC
						If aScan(aInvoices  , Substr(SC5->C5_ZZCODIN,1,At("-",SC5->C5_ZZCODIN)-1)) == 0
							aadd(aInvoices  , Substr(SC5->C5_ZZCODIN,1,At("-",SC5->C5_ZZCODIN)-1))
						Endif
					Endif
					//					If SM0->M0_CODIGO == '01' .and. SM0->M0_CODFIL == '02'
					If Alltrim(SM0->M0_CODFIL) == '0101' //EUROFINS RECIFE
						If ( Empty(aDest[16]) .or. !(Alltrim(aDest[16]) $ Alltrim(SC5->C5_ZZNFMAI)) )
							aDest[16] := Alltrim(SC5->C5_ZZNFMAI)
						Endif
					Endif
					// Personalizado - fim
				Else 
				...
				*/
				...
				DbSeek(xFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)
             	...
             	If !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
             		...
             		//cMensCli := If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(&("SC5->"+cFieldMsg))),&("SC5->"+cFieldMsg))+" "//Personalizado
					 cMensCli := If(FindFunction('CleanSpecChar'),CleanSpecChar(ConvType(Alltrim(&("SC5->"+cFieldMsg)),100)),ConvType(&("SC5->"+cFieldMsg),100))+" "  // Personalizado
					//ElseIf !AllTrim(SC5->C5_MENNOTA) $ cMensCli //Personalizado
					//cMensCli +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SC5->C5_MENNOTA)),AllTrim(SC5->C5_MENNOTA)) //Personalizado
				ElseIf !(If(FindFunction('CleanSpecChar'),CleanSpecChar(ConvType(AllTrim(SC5->C5_MENNOTA),250)),ConvType(AllTrim(SC5->C5_MENNOTA),250)) $ Alltrim(cMensCli))	//Personalizado
					cMensCli +=If(FindFunction('CleanSpecChar'),CleanSpecChar(ConvType(AllTrim(SC5->C5_MENNOTA),250)),ConvType(AllTrim(SC5->C5_MENNOTA),250))     	//Personalizado
				EndIf

				If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
					//cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(SC5->C5_MENPAD))),AllTrim(FORMULA(SC5->C5_MENPAD))) //Personalizado
					cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(ConvType(AllTrim(FORMULA(SC5->C5_MENPAD)),100)),ConvType(AllTrim(FORMULA(SC5->C5_MENPAD)),100))   //Personalizado
				EndIf
				...
				*/
				nRetDesc :=	Iif(nRetPis > 0, (cAliasSD2)->D2_VALPIS, 0) + Iif(nRetCof > 0, (cAliasSD2)->D2_VALCOF, 0) + ;
				Iif(nRetCsl > 0, (cAliasSD2)->D2_VALCSL, 0) + Iif(SF2->(FieldPos("F2_VALIRRF")) <> 0 .and. nRetIRRF > 0, (cAliasSD2)->D2_VALIRRF, 0) + ;
				Iif(SF2->(FieldPos("F2_BASEINS")) <> 0 .and. SF2->F2_BASEINS > 0, (cAliasSD2)->D2_VALINS, 0) + Iif(Len(aRetISS) >= nCont, aRetISS[nCont], 0)
				// Personalizado - fim
		...
		If lFirst
			...
			While SE1->(!Eof()) .And. SE1->E1_FILIAL == xFilial("SE1") .And. SE1->(E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM) == SF2->(F2_CLIENTE+F2_LOJA+F2_SERIE+F2_DOC)
				...
				If SE1->E1_TIPO == "NF "
					...
					//pcc
					nVlCSLL += iif(SE1->E1_CSLL>=SuperGetMv( "MV_ZZRTCSL"),SE1->E1_CSLL,0)
					nVlCOFI	+= iif(SE1->E1_COFINS>=SuperGetMv( "MV_ZZRTCOF"),SE1->E1_COFINS,0)
					nVlPIS += iif(SE1->E1_PIS>=SuperGetMv( "MV_ZZRTPIS"),SE1->E1_PIS,0)
					...
					While !Eof()
							...
							If WSE1->E1_TIPO == "IR-"
								nVlIRRF += WSE1->E1_VALOR
							Elseif WSE1->E1_TIPO == "PI-"
								nVlPIS += WSE1->E1_VALOR
							ElseIf WSE1->E1_TIPO == "CF-"
								nVlCOFI += WSE1->E1_VALOR
							ElseIf WSE1->E1_TIPO == "CS-"
								nVlCSLL += WSE1->E1_VALOR
							Endif

						WSE1->(dbSkip())
					Enddo

					nValorTit := nValorTit - (nVlIRRF + nVlPIS + nVlCOFI + nVlCSLL)

					dbCloseArea("WSE1")
					RestArea(aAreaSE1)

					AADD(aDuplic , {DtoC(SE1->E1_VENCTO) , nValorTit , Alltrim(SE1->E1_PARCELA)})
			...
			For nP:=1 To Len(aProcessos)
				cProcs += iif(nP==1,"",", ")+aProcessos[nP]
			Next
			//			If SM0->M0_CODIGO == '05'
			If Alltrim(SM0->M0_CODFIL) $ '0500|0501|0502|0503|0504' //ANNATEC
				For nI:=1 To Len(aInvoices)
					cInvs += iif(nI==1,"",", ")+aInvoices[nI]
				Next
			Endif

			If !Empty(cMensCli)
				cAux += Iif(Empty(cAux) , cMensCli , " " + cMensCli)
				cMensCli := ""
			Endif

			//			If (SM0->M0_CODIGO == '05' .and. xFilial("SD2") == "05")
			If Alltrim(SM0->M0_CODFIL) $ '0504' //Sao bernardo
				cNatOper := "Servicos de Analise"
				//			Elseif (SM0->M0_CODIGO == '01' .and. xFilial("SD2") == "02")
			elseIf Alltrim(SM0->M0_CODFIL) $ '0101' //Eurofins recife
				cNatOper := "Servicos de analise em amostras de produtos alimenticios. ||| Caro cliente, informamos que o recolhimento do ISSQN e de obrigatoriedade da Eurofins do Brasil. ||| Gentileza nao reter."
			Else
				cNatOper := "TESTES E ANÁLISES TÉCNICAS"
			Endif
			cMensCli += "||"

			If Alltrim(SM0->M0_CODFIL) $ '0500|0501|0502|0503|0504' //ANNATEC
				If Len(aProcessos) > 1
					cMensCli += "Numeros dos Processos: "+cProcs+"|"
				Else
					cMensCli += "Numero do Processo: "+cProcs+"|"
				Endif
			Endif
			If Alltrim(SM0->M0_CODFIL) $ '0500|0501|0502|0503|0504' //ANNATEC
				If Len(aInvoices) > 1
					cMensCli += "Numero das Invoices: "+cInvs+"||"
				Else
					cMensCli += "Numero da Invoice: "+cInvs+"||"
				Endif
			Endif

			If Len(aDuplic) > 0
				For nD:=1 to Len(aDuplic)
					cDuplic += "Vencto: "+aDuplic[nD][1]+;
					IIf(Alltrim(SM0->M0_CODFIL) $ '0500|0501|0502|0503|0504'," Valor Liquido: "+Alltrim(Transform(aDuplic[nD][2],"@E 999,999,999.99")),"")+;
					" Parcela: "+Iif(Empty(aDuplic[nD][3]),"UNICA",aDuplic[nD][3])+"  |"
				Next
			Else
				cDuplic := "|"
			Endif
			...
			//Personalizado Inicio
			//Solicitado por Adrielli e Renata - 05/11/2021 - Régis Ferreira Totvs IP Jundiai
			If cFilAnt == "0101"
...
/*			If SA1->A1_ZZTPCOB == "D" .and. Len(aDuplic) > 0 //removido em 23/05/19 por sergio braz ch. 2019051610036496
				cMensCli += "|"
				cMensCli += "Dados para Deposito|"
				cMensCli += "Banco "+SuperGetMV("MV_ZZNFSBC", ," ")+"|"
				cMensCli += "Agencia "+SuperGetMV("MV_ZZNFSAG", ," ")+"|"
				cMensCli += "Conta Corrente "+SuperGetMV("MV_ZZNFSCC", ," ")+"|"
			Endif */

			If !Empty(cLei) .And. !AllTrim(FORMULA(cLei)) $ cMensCli
				cMensCli += "|" + If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(cLei))),AllTrim(FORMULA(cLei)))
			EndIf

			//cNatOper := cMensCli+"||"+cAux
			//cMensCli := ""
			cMensCli += "||"+cAux
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf,aRetSF3,cMensCli ) //Personalizado
				cMensCli := "" // Personalizado
				cString	+= valores( aISSQN, aRetido, aTotal, aDest, if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ),aLeiTrp,lRecIrrf,aProd )
				//cString	+= faturas( aDupl )//Personalizado
...
/*/
//-----------------------------------------------------------------------
//static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3)	//Personalizado
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3,cMensCli)	//Personalizado
	...
	for nX := 1 to len( aProd )
		...
		EndIf
		*/
		cAux := cNatOper + cMensCli
		cString	+= "<discr>" + AllTrim(cAux)+ cCargaTrb + "</discr>"
		cMensCli := ""
		// Personalizado - fim		
