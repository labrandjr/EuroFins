...
/*/
//-----------------------------------------------------------------------
user function nfsPadrao( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local cLei	   :=SuperGetMV("MV_ZZLEINF", ,"") //COLOCA MENSAGEM DA LEI //Personalizado
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If cFilAnt == "0101"
					cMensCli += "||Local de Tributacao : Recife/PE."
				Endif
				//PERSONALIZADO
				If !Empty(cLei) .And. !AllTrim(FORMULA(cLei)) $ cMensFis
					cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(cLei))),AllTrim(FORMULA(cLei)))
				EndIf
				//Personalizado Fim
