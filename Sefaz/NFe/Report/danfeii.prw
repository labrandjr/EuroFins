...
#DEFINE MAXMENLIN  088					// Máximo de caracteres por linha de dados adicionais //Personalizado
...
Private cDesc			:= "" //Tratar a descrição do produto
//Personalizado Fim
...
//Personalizado Inicio
	//troca a descrição do produto pelo B5_DESCNFE
	if cFilAnt $ "0400" //Valido somente para a Alac
			cDesc := Upper(Alltrim(Posicione("SB5",1,xFilial("SB5")+Alltrim(oDet[nX]:_Prod:_cProd:TEXT),"B5_DESCNFE")))
		if Empty(Alltrim(cDesc))
			cDesc := Upper(Alltrim(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte)))			
		endif
	else
		cDesc := Upper(Alltrim(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte)))
	endif
	...
	//Personalizado
	//retirado do segundo item do aItens
	...
	//Personalizado inicio
	cAux     := AllTrim(SubStr(NoChar(cdesc,lConverte),(nMaxDes+1)))	
	//cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+1)))
	//Personalizado FIm
