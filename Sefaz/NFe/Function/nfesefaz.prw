...
//****************************************************************
//Para Personalizações no NFSEFAZ, buscar sempre por Personalizado
//****************************************************************
...
Local cNumDI    := ""  		// Personalizado - Régis Ferreira - Totvs IP
Local cEmisDI   := ""		// Personalizado - Régis Ferreira - Totvs IP
Local lImport   := .F. 		// Personalizado - Régis Ferreira - Totvs IP
					...
					If lContinua
						...
						if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( xFilial( "SFT" ) +cChave) )
						   ...
						   Endif	
							...
							If !lNatOper
								...
								Else
									//cNatOper += Iif(!Alltrim(SF4->F4_TEXTO)$cNatOper,"/ " + SF4->F4_TEXTO,"") // Personalizado Régis Ferreira - Totvs IP
									cNatOper += Iif(!Alltrim(SF4->F4_TEXTO)$cNatOper,"/ " + Alltrim(SF4->F4_TEXTO),"")	// Personalizado Régis Ferreira - Totvs IP
								Endif
							Else				   	
						...
						*/
						...
						Else
							aadd(aExp,{})
						EndIf
						/*Retirado personalização pois a função já é feita na rotina acima
						// Personalizado - Régis Ferreira - Totvs IP
						// Início
						If Substr((cAliasSD2)->D2_CF,1,1)=='7'
					    	aDados := {}
					    	aAdd(aDados,{"ZA02","ufEmbarq"  , SM0->M0_ESTENT })
				    		aAdd(aDados,{"ZA03","xLocEmbarq", SM0->M0_CIDENT })
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			endIf
			// Tratar dados complementares da importacao 
			// Personalizado - Régis Ferreira - Totvs IP
			// Início
		    cNumDI := ""
		    If AliasInDic("SZA")
				SZA->(dbSetOrder(5))
				If SZA->(dbSeek(xFilial("SZA")+SF1->(F1_FORNECE+F1_LOJA+F1_DOC+F1_SERIE)))
					cNumDI  := SZA->ZA_NUM_DI
					cEmisDI := DtoC(SZA->ZA_EMISSAO)
					nValorII  := 0
					nValorSis := 0
					nValorIPI := 0
					nValorPIS := 0
					nValorCOF := 0
					While !SZA->(Eof()) .and. SZA->(ZA_FILIAL+ZA_FORNECE+ZA_LOJA+ZA_NUMDOC+ZA_SERIE) == xFilial("SF1")+SF1->(F1_FORNECE+F1_LOJA+F1_DOC+F1_SERIE)
						nValorII  += SZA->ZA_VLRII
						nValorSis += SZA->ZA_SISCOME
						nValorIPI += SZA->ZA_VLRIPI
						nValorPIS += SZA->ZA_VLRPIS
						nValorCOF += SZA->ZA_VLRCOF
						SZA->(dbSkip())
					Enddo

					cMensFis += " Declaracao de Importacao No: "+cNumDI+" de "+cEmisDI+". "
					If SF1->F1_TIPO == "N"
						cMensFis += "PIS R$ "+Alltrim(Transform(nValorPIS,"@E 9999,999.99"))+;
					               	"  COFINS R$ "+Alltrim(Transform(nValorCOF,"@E 9999,999.99"))+;
					               	"  IPI R$ "+Alltrim(Transform(nValorIPI,"@E 9999,999.99"))+;
					               	"  SISCOMEX R$ "+Alltrim(Transform(nValorSis,"@E 9999,999.99"))+;
					               	"  IMP.IMPORT. R$ "+Alltrim(Transform(nValorII,"@E 9999,999.99"))+"."
	  				Endif
	  			Endif
	  		    // Caso o usuario nao tenha utilizado a rotina customizada, pega os dados do array aDi e SF1
	  		    lImport := .F.
				If Empty(cNumDI) .and. Len(aDI)>0 .and. Len(aDi[1])>0 .and. Len(aDi[1][4])>0 .and. ConvType(aDI[1][4][1]) == "I19"
					cNumDI  := ConvType(aDI[1][4][3])
					cEmisDI := DtoC(aDI[1][5][3])
			    	lImport := .T.
				EndIf
				If lImport
					cMensFis += " Declaracao de Importacao No: "+cNumDI+" de "+cEmisDI+". "
					...
					cMensFis += "PIS R$ "+Alltrim(Transform(SF1->F1_VALIMP6,"@E 9999,999.99"))+;
				               	"  COFINS R$ "+Alltrim(Transform(SF1->F1_VALIMP5,"@E 9999,999.99"))+;
				               	"  SISCOMEX R$ "+Alltrim(Transform(SF1->F1_FRETE,"@E 9999,999.99"))+;
				               	"  IMP.IMPORT. R$ "+Alltrim(Transform(SF1->F1_II,"@E 9999,999.99"))+"."


					//cMensFis += "PIS R$ "+Alltrim(Transform(SF1->F1_VALPIS,"@E 9999,999.99"))+;
				    //          	"  COFINS R$ "+Alltrim(Transform(SF1->F1_VALCOFI,"@E 9999,999.99"))+;
				    //         	"  SISCOMEX R$ "+Alltrim(Transform(SF1->F1_DESPESA,"@E 9999,999.99"))+;
				    //          	"  IMP.IMPORT. R$ "+Alltrim(Transform(SF1->F1_II,"@E 9999,999.99"))+"."
  					Endif
  				Endif
	  		Endif
...
If cVeramb >= "3.10"
	//If cTipocli  == "F" // Personalizado - Régis Ferreira - Totvs IP
	If cTipocli $ "F#X" // Personalizado - Régis Ferreira - Totvs IP
