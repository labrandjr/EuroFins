...
/*/{Protheus.doc} NFSe11606
Gera a NF Serviço de Recife / PE (IBGE = 11606)
@type function
@version 1.0
@author Ademar Fernandes Jr.
@since 10/04/2023
@link https://gkcmp.com.br (Geeker Company)
@see https://nfse.recife.pe.gov.br/capa.aspx
@see https://nfse.recife.pe.gov.br/rps.aspx
@see https://nfse.recife.pe.gov.br/documentos/verificacao.aspx
/*/
User Function NFSe11606(cNFSeFil,cNFSeNum,cNFSeSer,cNFSeCli,cNFSeLoj,cModNFSe,lPreview)
	Local cNFSe := ""
	...
	Default cNFSeCli := ""
	Default cNFSeLoj := ""
	Default cModNFSe := SuperGetMV("ZZ_11606MD",.F.,"000002")	//-Recife
	Default lPreview := .F.

	private cLogoEmpr   := SuperGetMV("ZZ_NFSESM0",.F.,"")	//-Logo da Empresa/Cliente
	private cLogoPref   := SuperGetMV("ZZ_11606LG",.F.,"")	//-Logo da Prefeitura Municipal
	private cLogoNFSe   := SuperGetMV("ZZ_11606L2",.F.,"")	//-Logo da NFSe da Prefeitura
...
Static Function Cabec()
	Local cRPS 			:= Alltrim(SF2->F2_NFELETR)
	...
	oPrint:SayBitmap(nLin+(SALTO_LIN+10), INI_COL+0050, Alltrim(cLogoPref), 800, 200)
	oPrint:SayBitmap(nLin+(SALTO_LIN+20), INI_COL+1150, Alltrim(cLogoNFSe), 600, 200)
	...
	nLin += SALTO_LIN
	oPrint:Say(nLin, aColImp[17]-20, SubStr(AllTrim(cNFSeNum),2,9), oFont16N)
...
Static Function Impostos()
	...
	nLin += SALTO_LIN
	oPrint:Say(nLin, INI_COL+10,	 "COFINS (R$)", 		oFont12)
	oPrint:Say(nLin, aColImp[03]+10, "CSLL (R$)", 			oFont12)
	oPrint:Say(nLin, aColImp[06]+10, "INSS (R$)", 			oFont12)
	oPrint:Say(nLin, aColImp[09]+10, "IRPJ (R$)",			oFont12)
	oPrint:Say(nLin, aColImp[12]+10, "PIS (R$)", 			oFont12)
	oPrint:Say(nLin, aColImp[15]+10, "Outras Retenções (R$)",oFont12)

	nLin += SALTO_LIN

	oPrint:Say(nLin, INI_COL+10, 	 AllTrim(Transform(cVlrCofins, 	PesqPict("SF2", "F2_VALCOFI"))), oFont12N)
	oPrint:Say(nLin, aColImp[03]+10, AllTrim(Transform(cVlrCSLL, 	PesqPict("SF2", "F2_VALCSLL"))), oFont12N)
	oPrint:Say(nLin, aColImp[06]+10, AllTrim(Transform(cVlrINSS, 	PesqPict("SF2", "F2_VALINSS"))), oFont12N)
	oPrint:Say(nLin, aColImp[09]+10, AllTrim(Transform(cVlrIR, 		PesqPict("SF2", "F2_VALIRRF"))), oFont12N)
	oPrint:Say(nLin, aColImp[12]+10, AllTrim(Transform(cVlrPIS, 	PesqPict("SF2", "F2_VALPIS"))),  oFont12N)
	oPrint:Say(nLin, aColImp[15]+10, AllTrim(Transform(cVlrOutRed,	PesqPict("SF2", "F2_VALPIS"))),  oFont12N)

	nLin += SALTO_LIN2 //SALTO_LIN
	oPrint:Line(nLin, INI_COL, nLin, aColImp[20])

	nLin += SALTO_LIN * 2
	oPrint:Say(nLin, aColImp[8]-10, "VALOR TOTAL DO SERVIÇO = R$ "+AllTrim(Transform(cVlrServ,PesqPict("SF3","F3_VALCONT"))), oFont16N)
...
Static Function OutrasInformacoes()
	...
	nLin += SALTO_LIN
	oPrint:Say(nLin, INI_COL+10, "- Esta NFS-e foi emitida com respaldo nas Leis 17.407/2008 e 17.408/2008.", oFont12)

	if SE2->(!Eof()) .And. SF2->(F2_SERIE+F2_DOC) == SE2->(E2_PREFIXO+E2_NUM) .And. SE2->E2_TIPO == "ISS"
		lDtBxIss := .T.
	endif
	nLin += SALTO_LIN
	oPrint:Say(nLin, INI_COL+10, "- O ISS referente a esta NFS-e foi recolhido"+iif( !lDtBxIss, "", " em "+Dtoc(SE2->E2_BAIXA)), oFont12)

	nLin += SALTO_LIN
	oPrint:Say(nLin, INI_COL+10, "- Esta NFS-e não gera crédito.", oFont12)

	nLin += SALTO_LIN
	oPrint:Say(nLin, INI_COL+10, "- Esta NFS-e substitui o RPS N. "+cRPS+" Serie "+cNFSerie+", emitido em "+cDtEmisRPS, oFont12)
...
Static Function PrintObserv()
	...
	nLin += SALTO_LIN
	oPrint:Say(nLin,INI_COL+10, "Codigo da Atividade Prestada: ", oFont14)
