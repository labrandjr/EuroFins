...
user function ntRFiscal()
	local cEmpX := "01"
	local cFilX := iif(lEurofins, "0100", "5000")
	local cQuery := ""

	ConOut("Inicio da rotina de schedule NTRFiscal")
	...
	while (cTRX)->(!eof())
		...
		cQuery += " 	 , F1_XDTBLOQ AS DT_BLOQ
		cQuery += " 	 , A2_NOME AS FORNECEDOR
		cQuery += " 	 , C7_XSOLICI AS SOLICIT
		cQuery += " 	 , (SELECT USR_EMAIL FROM SYS_USR USR WHERE USR.D_E_L_E_T_ = '' AND USR_ID  = CR_USER) AS APROV
        cQuery += "      , F1_XMOTRF AS MOTIVO
		...
		cQuery += "    AND CR_NIVEL = CASE WHEN LEFT(F1_APROV,2) = 'CR' THEN '02' ELSE '01' END
		...
		If  (cTRB)->(!eof())
			...
			cEmail   := alltrim((cTRB)->APROV)

			cHtml := '<!doctype html> ' + CRLF
			cHtml += ' <html> ' + CRLF
			cHtml += ' <head> ' + CRLF
			cHtml += ' <meta charset="utf-8"> ' + CRLF
			cHtml += ' <title>Eurofins - Bloqueio Código de Recebimento Fiscal (Tolerancia)</title> ' + CRLF
			...
			cHtml += ' 	<p class="titulo"><strong>(Recebimento Fiscal (Tolerancia Data))</strong></p> ' + CRLF
			cHtml += ' </div> ' + CRLF
			cHtml += ' <table width="90%" border="0" cellspacing="2"> ' + CRLF
			cHtml += '   <tbody> ' + CRLF
			cHtml += '     <tr> ' + CRLF
			cHtml += '       <td class="titTab" width="5%">Filial / Empresa</td> ' + CRLF
			...
			cHtml += '       <td class="titTab" width="10%">Classificacao NF</td> ' + CRLF
			cHtml += '       <td class="titTab" width="20%">Fornecedor</td> ' + CRLF
			cHtml += '       <td class="titTab" width="20%">Motivo</td> ' + CRLF
			...
			while (cTRB)->(!eof())
				...
				cHtml += '       <td class="itemTab" align="center">' + cValToChar(sToD((cTRB)->DT_BLOQ)) + '</td> ' + CRLF
				cHtml += '       <td class="itemTab">' + alltrim((cTRB)->FORNECEDOR) + '</td> ' + CRLF
				cHtml += '       <td class="itemTab">' + alltrim((cTRB)->MOTIVO) + '</td> ' + CRLF
			...
			U_SendMail(, cEmail, ;
				cCC,;
				'Notas Fiscais Bloqueio Código de Recebimento Fiscal (Tolerancia Data)',;
				cHtml,;
				'')
	...
	Reset Environment
	ConOut("Termino da rotina de schedule NTRFiscal.")
